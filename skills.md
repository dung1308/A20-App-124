...
## 8. Responsive Visibility Patterns
In complex "App Shell" layouts, certain navigational elements (like top bars) can be redundant on mobile where space is at a premium.

### Hiding Desktop Headers on Mobile
Use the `hidden` class as the base (mobile-first) and `md:flex` (or `lg:flex`) to show it only when there is enough horizontal space.

**Example:**
```javascript
/* Hidden on all phones/small tablets, shows at 768px+ */
<header className="hidden md:flex h-16 ...">
  <DesktopNav />
</header>
```

### Breaking Down Breakpoints
- **Phone (Portrait):** < 640px (`hidden` applies)
- **Phone (Landscape) / Small Tablet:** 640px - 767px (`sm:` breakpoint)
- **Tablet (iPad) / Laptop:** 768px+ (`md:` breakpoint)

By using `hidden md:flex`, we ensure that any device smaller than an iPad will not see the desktop header, relying instead on a mobile drawer/burger menu.
```