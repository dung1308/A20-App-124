...
def _ensure_audit_log_columns(engine, db_url: str) -> None:
    """Ensure audit_logs table has the latest PMF/audit columns."""
    try:
        ...
        if "route" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN route VARCHAR")
        if "response_time_ms" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN response_time_ms FLOAT")
        if "ai_resolved" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN ai_resolved BOOLEAN DEFAULT false")
        if "fallback" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN fallback BOOLEAN DEFAULT false")
        if "input_data" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN input_data TEXT")
        if "output_data" not in existing_columns:
            missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN output_data TEXT")
        if "judge_result" not in existing_columns:
            if db_url.startswith("sqlite"):
                missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN judge_result TEXT")
            else:
                missing_alterations.append("ALTER TABLE audit_logs ADD COLUMN judge_result JSON")

        if not missing_alterations:
            return

        with engine.connect() as conn:
            for statement in missing_alterations:
                logger.info(f"Applying missing audit_logs schema change: {statement}")
                conn.execute(text(statement))
            logger.info("✓ audit_logs schema updated with missing columns")
    except Exception as e:
        logger.warning(f"Could not migrate audit_logs schema automatically: {e}")


def _ensure_session_schema(engine, db_url: str) -> None:
    """Ensure chat_sessions and chat_messages have the required session schema."""
    try:
        inspector = inspect(engine)
        ...
        if "chat_sessions" in table_names:
            ...
            if "title" not in existing_columns:
                alterations.append("ALTER TABLE chat_sessions ADD COLUMN title VARCHAR DEFAULT 'Hội thoại mới'")
            if alterations:
                with engine.connect() as conn:
                    for statement in alterations:
                        logger.info(f"Applying missing chat_sessions schema change: {statement}")
                        conn.execute(text(statement))
                    logger.info("✓ chat_sessions schema updated with missing columns")

        if "chat_messages" in table_names:
            existing_columns = {col["name"] for col in inspector.get_columns("chat_messages")}
            if "session_id" not in existing_columns:
                with engine.connect() as conn:
                    statement = "ALTER TABLE chat_messages ADD COLUMN session_id VARCHAR"
                    logger.info(f"Applying missing chat_messages schema change: {statement}")
                    conn.execute(text(statement))
                    logger.info("✓ chat_messages schema updated with missing column session_id")
    except Exception as e:
        logger.warning(f"Could not migrate chat session schema automatically: {e}")
...
def init_database() -> None:
    ...
    try:
        ...
        _ensure_audit_log_columns(engine, db_url)
        _ensure_session_schema(engine, db_url)
