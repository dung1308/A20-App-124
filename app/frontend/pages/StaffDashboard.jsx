...
const StaffDashboard = () => {
    ...
    const fetchMetrics = async () => {
        ...
        try {
            const data = await api.getMetrics(168);
            setMetrics(data);
        } catch (err) {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const data = await api.getHandoffSummary(targetUser);
            setSummary(data.handoff_summary);
        } catch (err) {
            let message = err.message || "Đã xảy ra lỗi khi lấy dữ liệu từ máy chủ.";
            if (err.response?.status === 404) {
                message = "Không tìm thấy tóm tắt bàn giao cho học sinh này.";
            }
            if (err.response?.status === 403) {
                message = "Bạn không có quyền truy cập. Yêu cầu tài khoản Editor hoặc Admin.";
            }
            setError(message);
        } finally {
            setLoading(false);
    ...
    const handleEmailClick = async () => {
        ...
        // Log the action to the backend audit trail
        try {
            await api.logEmailSent(targetUser);
        } catch (err) { console.error("Audit logging failed", err); }
        
        // Proceed with opening the mail client
        window.location.href = `mailto:${targetUser}`;
    };
