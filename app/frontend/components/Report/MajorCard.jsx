      ...
      <div className="space-y-4">
        ...
        {(matchedSignals.length > 0 || tradeoffs.length > 0) && (
          <div className="space-y-3">
            {matchedSignals.length > 0 && (
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Matched signals</p>
                <div className="flex flex-wrap gap-2">
                  {matchedSignals.map((signal, index) => (
                    <span key={`${signal.label}-${index}`} className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-[10px] font-bold">
            ...
            {tradeoffs.length > 0 && (
              <div>
                <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-2">Tradeoffs to check</p>
                <ul className="space-y-1">
                  {tradeoffs.map((tradeoff, index) => (
                    <li key={`${tradeoff.label}-${index}`} className="text-[11px] text-slate-600 leading-snug">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {evidence.map((source, index) => (
            <span key={`${source.title}-${index}`} className="px-2 py-1 rounded border border-slate-200 bg-white text-[10px] font-black uppercase tracking-wider text-slate-500">
        ...
        {onAsk && (
          <button
            type="button"
            onClick={() => onAsk(major)}
            className="w-full px-4 py-2.5 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-widest"
          >
